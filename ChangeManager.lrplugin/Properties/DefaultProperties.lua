...
_t.isFinished = function( params )
    ...
    -- ###1 distribute to Exportant and ChangeManager...
    if app:lrVersion() >= 5 then
        local pickStatus = lrMeta:getRawMetadata( photo, 'pickStatus', cache, true )
    ...
    local keywords = lrMeta:getRawMetadata( photo, 'keywords', cache, true )
    if #keywords > 0 then
    ...
    local creator = lrMeta:getFormattedMetadata( photo, 'artist', cache, true )
    ...
    --[[
    local copyright = lrMeta:getFormattedMetadata( photo, 'copyright', cache, true )
